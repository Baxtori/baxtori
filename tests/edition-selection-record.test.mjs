...
test("a complete published selection record validates", () => {
  const validated = validateEditionSelectionRecord(selection(), stories);
...
test("every published story must appear exactly once", () => {
  assert.throws(() => validateEditionSelectionRecord(selection({
    included: selection().included.slice(0, 1),
    plannedMinutes: 5,
  }), stories), /account for every published story/);

  assert.throws(() => validateEditionSelectionRecord(selection({
    included: [selection().included[0], selection().included[0]],
    plannedMinutes: 10,
  }), stories), /duplicate alpha/);
});

test("selection records reject unknown priorities", () => {
  const invalid = selection();
  invalid.deferred[0].priority = "urgent";
  assert.throws(() => validateEditionSelectionRecord(invalid, stories), /unknown priority/);
});

test("planned minutes must equal the included estimates", () => {
  assert.throws(() => validateEditionSelectionRecord(selection({ plannedMinutes: 10 }), stories), /must equal included estimates/);
});

test("deferred and excluded findings cannot reuse an ID", () => {
  const invalid = selection();
  invalid.excluded[0].id = "gamma";
  assert.throws(() => validateEditionSelectionRecord(invalid, stories), /duplicate gamma/);
});

test("quiet and inaccessible repositories cannot overlap", () => {
  const invalid = selection();
  invalid.inaccessible[0].repository = "teamleaderleo/quiet";
  assert.throws(() => validateEditionSelectionRecord(invalid, stories), /duplicate teamleaderleo\/quiet/);
});

test("repository decisions require canonical owner/name values", () => {
  const invalid = selection();
  invalid.quiet[0].repository = "not-a-repository";
  assert.throws(() => validateEditionSelectionRecord(invalid, stories), /owner\/name repository/);
});
