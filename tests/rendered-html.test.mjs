...
test("keeps GitHub credentials and feedback storage behind the server session", async () => {
  ...
  assert.match(startRoute, /cookieHeader\(request, STATE_COOKIE, state/);
  assert.doesNotMatch(startRoute, /redirect_uri/);
  assert.match(callbackRoute, /sealSession/);
  assert.match(callbackRoute, /validateGitHubOAuthState/);
  assert.match(callbackRoute, /state === expectedState/);
  assert.match(callbackRoute, /githubOAuthTokenBody/);
  assert.doesNotMatch(callbackRoute, /redirect_uri/);
  ...
  assert.match(feedbackStateRoute, /getGitHubIdentitySession/);
  assert.match(feedbackReviewsRoute, /getGitHubIdentitySession/);
  assert.match(feedbackTopicsRoute, /getGitHubIdentitySession/);
  assert.match(feedbackQuestionsRoute, /getGitHubIdentitySession/);
  assert.match(feedbackAccountRoute, /getGitHubIdentitySession/);
...
test("requires GitHub authentication before repository access", async () => {
  ...
  const topicResponse = await render("/api/feedback/topics", { method: "POST" });
  assert.equal(topicResponse.status, 401);

  const questionResponse = await render("/api/feedback/questions", { method: "POST" });
