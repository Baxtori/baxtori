...
const mappedRepositories = new Set(mapRegistry.maps.map((entry) => canonicalRepository(entry.repository)));
...
for (const repository of scope.repositories) {
  ...
  if (!fullName?.includes("/") || !repository.name || scheduled.has(fullName)) {
    throw new Error(`Invalid or duplicate scoped repository: ${repository.fullName}`);
  }
  if (repository.fullName !== fullName) {
    throw new Error(`${repository.fullName} is a legacy repository alias. Use ${fullName} in active review scope.`);
  }
  ...
  if ((repository.mapStatus === "mapped") !== mappedRepositories.has(fullName)) {
...
const configured = new Set(sources.repositories.map((repository) => {
  const fullName = canonicalRepository(repository.fullName);
  if (repository.fullName !== fullName) throw new Error(`${repository.fullName} is a legacy collector alias. Use ${fullName}.`);
  return fullName;
}));
